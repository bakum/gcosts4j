...
public class GraphPatterns {
     ...
     * @return new graph
     */
    public Gcosts<Node, Flow> getAccountingGraph(boolean withData) {
        //Создаем узлы графа
        ...
        fp1_231.setCost(!withData ? new Costs() : new Costs(100.0, 10.0));
        fp1_231.setType(FlowType.PRIMARY);
        g.setEdgeWeight(fp1_231, fp1_231.getAmount());

        Flow f91_231 = (Flow) g.addEdge(n91, n231); //Первичные затраты на себестоимость (ОПЗ)
        f91_231.setCost(!withData ? new Costs() : new Costs(200.0, 1.0));
        f91_231.setType(FlowType.PRIMARY);
        ...
        f231_231_begin.setCost(!withData  ? new Costs() : new Costs(2.0, 10.0));
        ...
        f231_231_end.setCost(!withData ? new Costs() : new Costs(0.0, -10.0));
        f231_231_end.setDirection(FlowDirection.END_VALUE);
        g.setEdgeWeight(f231_231_end, f231_231_end.getAmount());

        Flow f26_26_begin = (Flow) g.addEdge(n26, n26); //Начальные остатки готовой продукции
        f26_26_begin.setCost(!withData ? new Costs() : new Costs(2.0, 10.0));
        f26_26_begin.setDirection(FlowDirection.BEGIN_VALUE);
        g.setEdgeWeight(f26_26_begin, f26_26_begin.getAmount());

        Flow f26_26_end = (Flow) g.addEdge(n26, n26); //Конечные остатки готовой продукции
        f26_26_end.setCost(!withData ? new Costs() : new Costs(0.0, -10.0));
        f26_26_end.setDirection(FlowDirection.END_VALUE);
        g.setEdgeWeight(f26_26_end, f26_26_end.getAmount());

        Flow f231_26 = (Flow) g.addEdge(n231, n26); //Выпуск продукции
        f231_26.setCost(!withData ? new Costs() : new Costs(0.0, 10.0));
        g.setEdgeWeight(f231_26, f231_26.getAmount());

        Flow f26_901 = (Flow) g.addEdge(n26, n901); //Реализация продукции
        f26_901.setCost(!withData ? new Costs() : new Costs(0.0, 10.0));
